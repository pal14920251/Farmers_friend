...
from language_translation.text_generation import generate_comprehensive_crop_report
from language_translation.text_generation import generate_report_from_template
...
# load session results json file 
...
with tab1:
        ...
        # ---------------------------------------------
    # 🌐 LANGUAGE SELECTION FOR BHASHINI TRANSLATION
    # ---------------------------------------------
    st.markdown("---")
    ...
    selected_lang = st.selectbox("Choose Language", options=list(readable_langs.keys()))
    ...
    if tgt_lang != "eng_Latn":
        try:
            from language_translation.text_generation import translate_large_text
...
with tab2:
    ...
    if uploaded_file:
        ...
        if st.button("Predict Disease"):
            try:
                # -------------------------
                # 1️⃣ Predict the disease
                # -------------------------
                disease = predict_disease(img)
                st.success(f"**Predicted Disease:** {disease}")

                # -----------------------------
                # 2️⃣ Load template & generate
                # -----------------------------
                session_data = {
                    "identified_disease_or_pest_name": disease
                }

                disease_report = generate_report_from_template(
                    prompt_file="/data1/home/anumalas/GENAI-PROJECT/UI/language_translation/disease_prompt.json",
                    template_key="disease_treatment_prompt",
                    session_data=session_data
                )

                # -----------------------------
                # 3️⃣ Show English report
                # -----------------------------
                st.subheader("📘 Disease Advisory (English)")
                st.write(disease_report)

                # -----------------------------
                # 4️⃣ Language selection (optional)
                # -----------------------------
